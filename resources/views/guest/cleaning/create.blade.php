...
<body>
    <div class="container mt-4">
        ...
        <form action="{{ route('guest.cleaning.store') }}" method="POST" class="card p-4">
            @csrf
                ...
                <select class="form-select @error('reservation_id') is-invalid @enderror" name="reservation_id"
                    ...
                    @foreach ($reservations as $reservation)
                        <option value="{{ $reservation->id }}"
                            ...
                            Room {{ $reservation->room->room_number }} - {{ $reservation->check_in->format('M d, Y') }}
                            to {{ $reservation->check_out->format('M d, Y') }}
                        </option>
                    @endforeach
                </select>
            ...
            <div class="mb-3">
                ...
                <select class="form-select @error('request_type') is-invalid @enderror" name="request_type"
                    ...
                    <option value="towels" {{ old('request_type') == 'towels' ? 'selected' : '' }}>Towel Replacement
                    </option>
                    <option value="cleaning" {{ old('request_type') == 'cleaning' ? 'selected' : '' }}>Room Cleaning
                    </option>
            ...
            <div class="d-flex gap-2">
                ...
                <a href="{{ route('guest.cleaning.index') }}" class="btn btn-secondary">View My Requests</a>
            </div>
        </form>
    </div>
</body>
